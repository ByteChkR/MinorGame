...
namespace MinorGame.mapgenerator
{
    public class TileCreator
    {
        ...
        public static GameObject CreateCube(Vector3 position, Vector3 scale, Quaternion rotation, Texture texture, int mass = -1)
        {
            ...
            box.AddComponent(new MeshRendererComponent(GameScene.TextureShader, cube, texture, 1, false));
        ...
        public static GameObject CreateSphere(Vector3 position, Vector3 scale, Quaternion rotation, Texture texture, int mass = -1)
        {
            ...
            box.Scale = scale;
            box.Rotation = rotation;
            Mesh sphere = MeshLoader.Prefabs.Sphere;
            box.AddComponent(new MeshRendererComponent(GameScene.TextureShader, sphere, texture, 1, false));
            Vector3 bounds = scale * 2;
            Collider coll;
            if (mass == -1)
            {
